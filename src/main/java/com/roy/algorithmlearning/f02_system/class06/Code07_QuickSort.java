...
public class Code07_QuickSort {
    ...
    public static void quickSort(int[] nums) {
        ...
        queue.add(new Operation(0, partitioned[0]));
        queue.add(new Operation(partitioned[1], nums.length-1));
        while (!queue.isEmpty()) {
            Operation operation = queue.poll();
            int[] partitioned1 = partition(nums, operation.start, operation.end);
            if (partitioned1[0] != partitioned1[1]) {
                queue.add(new Operation(operation.start, partitioned1[0]));
                queue.add(new Operation(partitioned1[1], operation.end));
            }
        }
    }
